...
impl ChallengeTrait for Md5 {
  ...
  fn new(input: Self::Input) -> Self {
    Md5 { complexity: input.complexity, message: input.message}
  }
  ...
  fn solve(&self) -> Self::Output {
    for i in 0..std::u64::MAX {
      ...
      let hashcode = format!("{:0hashcode_size$X}", md5::compute(("".to_owned() + &seed + &self.message).into_bytes()));

      let seed_u64= u64::from_str_radix(&seed, 16).unwrap();

      let current_answer = MD5HashcashOutput { seed: seed_u64, hashcode };
      if self.verify(&current_answer) { return current_answer }
    }
