use std::{net::{TcpListener, TcpStream}, io::{Error, Read, Write, self}};
...
impl ServerManager {
  ...
  pub fn new(_args: Vec<String>) -> Result<ServerManager, Error> {
    ...
    Ok(ServerManager { listener, timeout: 2 })
  }
    ...
    for stream in listener.incoming() {
      ...
      match stream {
        Ok(stream) => {
          ...
          self.write_message(&stream, SentMessageServer::Welcome(Welcome { version: 1 }))?;
          self.read_message(&stream)?;
        }
        Err(error) => {
          println!("Error: {}", error);
  ...
  fn read_message(&self, mut stream: &TcpStream) -> Result<ReceivedMessageServer, Error> {
    ...
    stream.read_exact(size_buffer .as_mut()).expect("Server message failed to be read");

    let message_size = u32::from_be_bytes(size_buffer ); 

    let mut message_buffer  = vec![0u8; message_size as usize]; 
    stream.read_exact(message_buffer .as_mut()).expect("Server message failed to be read"); 

    let result = String::from_utf8_lossy(&message_buffer); 
    let received_message = serde_json::from_str::<ReceivedMessageServer>(&result).expect("Server message failed to be deserialized");
  ...
  fn write_message(&self, mut stream: &TcpStream, message_to_send: SentMessageServer) -> io::Result<()> {
    ...
    stream.write(&serialized_size.to_be_bytes()).expect("The message size failed to be sent");
    stream.write(&serialized.as_bytes()).expect("The message failed to be sent");
